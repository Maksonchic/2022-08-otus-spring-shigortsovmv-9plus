...
public class BookDto {
    ...
    public static BookDto createDto(final Book book, boolean isAuthorContains) {
        ...
        List<CommentDto> commentDtos = book.getComments().stream().map(CommentDto::createDto).collect(Collectors.toList());
        return new BookDto(
                book.getId(),
                book.getTitle(),
                book.getPageCount(),
    ...
    public static Book createEntity(final BookDto bookDto) {
        return new Book(
                ...
                bookDto.getComments().stream().map(CommentDto::createEntity).collect(Collectors.toList()));
    }
